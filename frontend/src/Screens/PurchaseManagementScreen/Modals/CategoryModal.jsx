...
import { addCategory, deleteCategory, fetchCategories } from "../../../apis/categories";
  ...
  const CategoryModal = () => {
    ...
    const { categoryInto, addCategoryApiStatus, error, success, deleteCategoryApiStatus } =
      useSelector((state) => state.categories);
  
    useEffect(() => {
      if (
        addCategoryApiStatus === ApiStates.success ||
        deleteCategoryApiStatus === ApiStates.success
      ) {
      ...
      <>
        ...
        <BasicModal open={openModal} handleClose={handleCloseModal}>
          ...
          <Stack direction="row" sx={{ py: 3 }} justifyContent="space-between">
            ...
            <CyanOutlineButton
              variant="outlined"
              disabled={addCategoryApiStatus === ApiStates.pending}
              onClick={handleAddCategory}
            >
              {addCategoryApiStatus === "pending" ? (
